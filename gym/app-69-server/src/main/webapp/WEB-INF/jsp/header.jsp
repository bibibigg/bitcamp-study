...
<div class="header-container">
    ...
    <a href='/app/member/list' class="header-link">회원</a>
    <a href='/app/board/list?category=1' class="header-link">게시글</a>
    <a href='/app/board/list?category=2' class="header-link">운동일지</a>

    <c:choose>
        <c:when test="${empty sessionScope.loginUser}">
        <a href='/app/auth/login' class="header-link">로그인</a>
        </c:when>
        <c:otherwise>
        ...
        ${loginUser.name} <a href='/app/auth/logout' class="header-link">로그아웃</a>
        </c:otherwise>
    </c:choose>
</div>
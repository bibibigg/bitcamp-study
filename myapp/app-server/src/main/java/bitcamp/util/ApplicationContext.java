...
// 그 리턴 값을 컨테이너에 보관한다.
// - 자바 설정 클래스 (예: AppConfig)에서 @componentScan 애노테이션을 찾아서 정보를 알아낸다.
...
public class ApplicationContext {
  ...
  private void createBeans(String basePackage) throws Exception {
    ...
    // 4) 패키지 디렉토리 안에 들어있는 파일이름이나 하위 디렉토리 이름을 읽을 도구를 준비한다.
    BufferedReader dirReader = new BufferedReader(new InputStreamReader(dirInputStream));

    // 5) 디렉토리 리더를 통해 해당 디렉토리에 들어 있는 하위 디렉토리 또는 파일을 이름을 알아낸다
    ...
    // // Class.forName()을 사용하여 클래스 메모리(Method Area)에 로딩한다.
    ...
    // return null;
    // }
    // }).collect(Collectors.toSet()); // 스트림을 수행하여 최종 결과물은 Set 컬렉션에 담에서 리턴하라고 명령한다.

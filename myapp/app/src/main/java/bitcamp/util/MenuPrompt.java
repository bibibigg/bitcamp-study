...
public class MenuPrompt extends Prompt {
  ...
  public String inputMenu() {
    ...
    while (true) {
      ...
      } else if (command.equals("menu")) { // command가 "menu"일시 printmenu실행
        this.printMenu();
      } else if (findMenuItem(command) == null) { // command가 널값일시 메뉴번호가 옳지 않음
  ...
  private String findMenuItem(String command) {
    ...
    // command에 해당하는 메뉴가 있다면 그 메뉴 이름은 리턴하고
    // 없다면 원래 null을 리턴한다.

...
public class App {
  ...
  private <T extends CsvObject> void loadCsv(String filename, List<T> list, Class<T> clazz) {
    try {
      ...
      while ((line = in.readLine()) != null) {
        ...
        // list.add(Member.fromCsv(line)); // 직접 스티택 메서드 호출
      }
  ...
  private void saveCsv(String filename, List<? extends CsvObject> list) { // ? 는 List에 들어갈것이 결정되지
                                                                          // 않았다는 뜻
    try {
      ...
      for (CsvObject obj : list) {
        ...
        // Board나Member 클래스에 필드가 추가/변경/삭제되더라도
        // 여기 코드를 변경할 필요가 없다.

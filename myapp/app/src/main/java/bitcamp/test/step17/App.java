...
// 5) 출력 기능을 별도의 메서드로 분류(중첩클래스; 스태틱 클래스)
...
// 12) 클래스를 유지보수 하기 쉽게 별도 소스 파일로 분류
// 13) 클래스를 유지보수 하기 쉽게 패키지로 분류: import, public
// 14) 외부접근 차단과 값 꺼내기 : private, gatter
// 15) 프로그래밍의 일관성을 위해 보통 다른 필드에 대해서도 gatter를 만들고 사용한다.
// 16) 필드의 직접 접근을 막고 setter를 정의하는 이유
// 17) 필드의 직접 접근을 막기 : 인스턴스 변수에 유호한 무효한 값이 저장되지 않게 하기 위해
// 17) => getter 정의 : 값을 꺼낼 때 사용
...
public class App {
  ...
  public static void main(String[] args) {
    ...
    scores[length++] = new Score("홍길동", 100, 1000, 100);
